...
The lab never uses future prices to decide whether a past signal existed.
A signal is recorded at t with features known at t; outcome columns are filled
only when enough later bars exist. Development and out-of-sample reports are
split chronologically so the dashboard can show whether an observed edge
survives a time holdout.
"""
...
CREATE INDEX IF NOT EXISTS idx_signals_scanner_ticker_time
ON signals(scanner, ticker, signal_time);
...
def record_signals(payload: dict[str, Any]) -> int:
    """Record the current scan snapshot. Duplicate timestamp/symbol signals are ignored."""
    rows: list[tuple[Any, ...]] = []
    ...
    for scanner_name, items in (("A", payload.get("scanner_a", [])), ("B", payload.get("scanner_b", []))):
        for x in items:
            ...
            if not t:
                continue
            key = signal_key(scanner_name, t, ts)
            features = {k: v for k, v in x.items() if k not in {"ticker", "score", "probability"}}
            rows.append((
                key, ts, scanner_name, t,
...
def _fill_for_signal(row: sqlite3.Row, df: pd.DataFrame, c: sqlite3.Cursor) -> int:
    ...
    idx = df.index
    # Use first market bar strictly AFTER signal time.
    pos = int(idx.searchsorted(ts, side="right"))
    filled = 0
    if pos >= len(df):
        return 0
    entry_close = float(row["close_at_signal"] or df["Close"].iloc[pos-1]) if pos > 0 else float(df["Close"].iloc[pos])
    for h in HORIZONS:
        if pos + h >= len(df):
            continue
        existing = c.execute("SELECT 1 FROM outcomes WHERE signal_id=? AND horizon=?", (row["id"], h)).fetchone()
        if existing:
            continue
        ...
        c.execute(
            ...
            (row["id"], h, idx[pos+h].isoformat(), future_close, ret*100, int(ret > 0), mfe*100, mae*100),
        )
        filled += 1
    return filled
...
def fill_outcomes() -> int:
    with connect() as c:
        ...
        for row in rows:
            ...
            df = cache[row["ticker"]]
            if df is not None:
                total += _fill_for_signal(row, df, c)
    return total
...
def _metrics(values: np.ndarray) -> dict[str, Any]:
    ...
    if len(v) == 0:
        return {"n": 0}
    prob = float(np.mean(v > 0))
    low, high = bootstrap_ci((v > 0).astype(float), np.mean, n_resamples=1000, mean_block=min(7, len(v))) if len(v) >= 20 else (None, None)
    return {
        "n": int(len(v)),
        "positive_rate": prob,
        "mean_return_pct": float(np.mean(v)),
        "median_return_pct": float(np.median(v)),
        "best_return_pct": float(np.max(v)),
        "worst_return_pct": float(np.min(v)),
        "mean_abs_return_pct": float(np.mean(np.abs(v))),
...
def summarize(test_fraction: float = 0.30) -> dict[str, Any]:
    with connect() as c:
        df = pd.read_sql_query(
            "SELECT s.scanner,s.ticker,s.score,s.probability,s.signal_time,o.horizon,o.return_pct,o.max_favorable_excursion_pct,o.max_adverse_excursion_pct FROM signals s JOIN outcomes o ON o.signal_id=s.id",
            c,
        )
    if df.empty:
        return {"signals": 0, "horizons": {}}
    df["signal_time"] = pd.to_datetime(df["signal_time"], utc=True)
    cutoff = df["signal_time"].quantile(1.0 - test_fraction) if len(df) >= 20 else df["signal_time"].max()
    result: dict[str, Any] = {"signals": int(df[["scanner","ticker","signal_time"]].drop_duplicates().shape[0]), "cutoff": cutoff.isoformat()}
    for scanner in ["A", "B"]:
        ...
        for h in HORIZONS:
            ...
            oos = hp[hp.signal_time >= cutoff]
            result[scanner]["out_of_sample"][str(h)] = _metrics(oos.return_pct.to_numpy(float))
        # Score buckets are intentionally descriptive, not tuned on outcomes.
