...
Default mode requires NO proprietary API keys. Public yfinance/Yahoo data is
used for history, screening, metadata and listed option chains. FINRA is an
optional authoritative short-interest enrichment. Missing provider data is
...
def metrics(df: pd.DataFrame) -> dict[str, float]:
    ...
    return {"price": px, "z20": z20, "z50": z50, "rsi14": float(rsi(c).iloc[-1]),
            "distance_52w": dist52 * 100, "relative_volume": rv,
            "momentum_1d": ret1 * 100, "momentum_3d": ret3 * 100,
            "momentum_5d": ret5 * 100, "momentum_20d": ret20 * 100,
            "atr_fraction": atr_frac, "avg_volume_20": avgv,
...
def state_signature(df: pd.DataFrame, pos: int) -> np.ndarray | None:
    ...
    rr = rsi(c); hi = h.rolling(252, min_periods=20).max(); dist = (hi - c) / hi
...
def black_scholes_gamma(spot: float, strike: float, t: float, rate: float, iv: float) -> float:
    if min(spot, strike, t, iv) <= 0: return 0.0
...
def _years_to_expiry(expiration: str, as_of: datetime | None = None) -> float:
    """Actual remaining time to the 16:00 ET expiration close, not a fixed 30d proxy."""
    now = as_of or datetime.now(timezone.utc)
    exp_date = datetime.strptime(str(expiration), "%Y-%m-%d").date()
    close_et = datetime(exp_date.year, exp_date.month, exp_date.day, 16, 0,
...
def options_snapshot(symbol: str, spot: float) -> dict:
    """Public yfinance options data, finalist-only, with expiry-aware gamma."""
    try:
        t = yf.Ticker(symbol)
        expirations = list(t.options or [])
        if not expirations: return {"status": "NO_OPTIONS"}
        use_exp = expirations[:3]
        calls, puts = [], []
        for exp in use_exp:
            chain = t.option_chain(exp)
            calls.append(chain.calls.copy()); puts.append(chain.puts.copy())
        ...
        if cdf.empty and pdf.empty: return {"status": "NO_DATA"}

        def liquidity(frame):
            if frame.empty: return {"oi": 0, "vol": 0, "spread": None}
            ...
            return {"oi": int(oi.sum()), "vol": int(vol.sum()), "spread": float(spread.median()) if not spread.empty else None}

        cl, pl = liquidity(cdf), liquidity(pdf)
        gex = 0.0; contracts_priced = 0
        ...
        for exp, cframe, pframe in zip(use_exp, calls, puts):
            ...
            for frame, sign in ((cframe, 1.0), (pframe, -1.0)):
                for _, r in frame.iterrows():
                    ...
                    if strike is None or iv is None or oi is None or oi <= 0: continue
        ...
        oi_total = cl["oi"] + pl["oi"]; vol_total = cl["vol"] + pl["vol"]
        ...
        return {
            ...
            "expirations_checked": use_exp, "call_oi": cl["oi"], "put_oi": pl["oi"],
            "call_volume": cl["vol"], "put_volume": pl["vol"], "oi_total": oi_total, "volume_total": vol_total,
            ...
            "net_dollar_gamma": round(gex, 0), "gamma_contracts_priced": contracts_priced,
...
def scan_a(signal_time: str) -> list[dict]:
    ...
    for item in universe:
        ...
        if sector in EXCLUDED_SECTORS or industry in EXCLUDED_SECTORS: continue
        f = metrics(df)
        if f["price"] < MIN_PRICE or f["avg_volume_20"] * f["price"] < MIN_AVG_DOLLAR_VOLUME: continue
        current = state_signature(df, len(df) - 1)
        prob, n, outcomes = historical_analog_probability(df, current)
        ext = np.clip(f["z20"] / 3.0, 0, 1); near = np.clip(1 - f["distance_52w"] / 8.0, 0, 1)
        rfac = np.clip((f["rsi14"] - 55) / 25.0, 0, 1); rv = np.clip((f["relative_volume"] - 1) / 3.0, 0, 1)
        ...
        results.append({"ticker": symbol, "name": item.get("name", symbol), "sector": sector, "industry": industry,
                        "leverage": item.get("leverage"), "price": round(f["price"], 4), "score": round(float(score), 1),
                        "reversal_score": round(float(score), 1), "z20": round(f["z20"], 2), "z50": round(f["z50"], 2),
                        "rsi14": round(f["rsi14"], 1), "distance_52w": round(f["distance_52w"], 2),
                        "relative_volume": round(f["relative_volume"], 2), "momentum_5d": round(f["momentum_5d"], 2),
                        "market_cap": market_cap, "avg_dollar_volume": round(f["avg_volume_20"] * f["price"]),
...
def scan_b(signal_time: str) -> list[dict]:
    ...
    try:
        ...
        if Path(p).exists():
            f = pd.read_csv(p)
            for _, r in f.iterrows(): finra[clean_symbol(r.get("ticker"))] = r.to_dict()
    ...
    for q in quotes:
        ...
        f = metrics(df); price = price or f["price"]
        if not price or price < MIN_PRICE: continue
        dollar_vol = f["avg_volume_20"] * price
        if dollar_vol < MIN_AVG_DOLLAR_VOLUME: continue
        if si_pct is None or si_pct < 5: continue
        if f["relative_volume"] < 1.25 or f["momentum_5d"] <= 0: continue
        ...
        change_s = np.clip(max(si_change or 0, 0) / 50, 0, 1); vol_s = np.clip(f["relative_volume"] / 5, 0, 1)
        mom_s = np.clip(max(f["momentum_5d"], 0) / 15, 0, 1)
        score = 100 * (0.30 * crowd + 0.15 * dtc_s + 0.15 * change_s + 0.22 * vol_s + 0.18 * mom_s)
        candidates.append({"ticker": s, "score": round(float(score), 1), "squeeze_score": round(float(score), 1),
                           ...
                           "short_data_source": "FINRA" if s in finra else "YAHOO_PUBLIC_SCREENER", "signal_time": signal_time})
    return sorted(candidates, key=lambda x: x["score"], reverse=True)[:40]


def enrich_options(items: list[dict]) -> list[dict]:
    for x in items[:FINALISTS_FOR_OPTIONS]: x["options"] = options_snapshot(x["ticker"], float(x["price"]))
    return items


def main():
    ts = now_utc(); a = enrich_options(scan_a(ts)); b = enrich_options(scan_b(ts))
    payload = {"generated_at": ts, "status": "LIVE — ZERO-KEY MODE",
               "data_sources": {"price_history": "yfinance public data", "market_screener": "Yahoo/yfinance public screener",
                                "company_metadata": "Yahoo/yfinance public info", "options": "yfinance public option chains",
               ...
               "scanner_a": a, "scanner_b": b, "signal_lab": "ACTIVE",
               "alerts_enabled": bool(os.getenv("SMTP_HOST") and os.getenv("ALERT_EMAIL_TO")),
               "methodology": {"A": "52-week proximity + extension state + RSI + relative volume + leakage-safe historical analogs.",
               ...
               "notes": ["ZERO-KEY MODE is fully usable without FINRA credentials.",
    ...
    try:
        ...
        record_signals(payload); lab = write_lab_report()
        payload["signal_lab_summary"] = {"signals": lab.get("signals", 0), "cutoff": lab.get("cutoff"),
